...
from proyecto_grado.modeling.baselines import (
    BASELINE_MODELS,
    predict_baseline,
...
from proyecto_grado.modeling.xgboost_models import (
    XGBOOST_MODELS,
    predict_xgboost,
    validate_xgboost_model_names,
)


def _validate_all_model_names(model_names: list[str]) -> None:
    """Validate a mixed list of baseline and XGBoost model names."""
    baseline_names = [m for m in model_names if m in BASELINE_MODELS]
    xgboost_names = [m for m in model_names if m in XGBOOST_MODELS]
    unknown = sorted(set(model_names) - set(BASELINE_MODELS) - set(XGBOOST_MODELS))
    if unknown:
        raise ValueError(f"Modelos no soportados: {unknown}")
    if baseline_names:
        validate_model_names(baseline_names)
    if xgboost_names:
        validate_xgboost_model_names(xgboost_names)


def _diagnostics_records(
    ...
    """Flatten per-route XGBoost fit diagnostics into report-ready records."""
    records: list[dict[str, object]] = []
    for route, diag in route_diagnostics.items():
        records.append(
            {
                "run_id": run_id,
                "target": target,
                ...
                "fold": str(fold),
                "route": int(route),
                "n_estimators_used": diag.n_estimators_used,
                "best_iteration": diag.best_iteration,
                "n_train_rows": diag.n_train_rows,
                "n_early_stop_rows": diag.n_early_stop_rows,
                "importance_gain": dict(diag.importance_gain),
            }
        )
    return records
...
def _prediction_frame(
    ...
    fallback_level: pd.Series | None = None,
) -> pd.DataFrame:
    ...
    out = pd.DataFrame(
        {
            ...
            "fallback_level": (
                fallback_level.astype(str).to_numpy()
...
def predict_window(
    ...
    operational_day_start: str = DEFAULT_OPERATIONAL_DAY_START,
    ...
    """Predict one temporal evaluation window for all requested models.

    Baseline models (``BASELINE_MODELS``) go through ``predict_baseline`` and
    keep their exact prior behavior. XGBoost configurations (``XGBOOST_MODELS``)
    are trained per route inside this window and, when ``diagnostics`` is
    passed, their per-route fit diagnostics (trees selected, gain importance)
    are appended to it in place.
    """
    if int(horizon) != 1:
        raise NotImplementedError("Esta fase solo soporta horizon=1")
    model_names = list(model_names)
    _validate_all_model_names(model_names)
    ...
    for model_name in model_names:
        ...
        if model_name in BASELINE_MODELS:
            ...
            fallback_level = meta["fallback_level"] if meta is not None else None
        else:
            y_pred, xgb_meta = predict_xgboost(
                model_name,
                train_df,
                predict_df,
                target_col=target_col,
...
def run_baseline_backtesting(
    ...
    Returns ``(predictions, metrics_by_fold, metrics_comparison, folds_frame,
    xgboost_diagnostics)``. ``xgboost_diagnostics`` has one row per
    (fold, split, model, route) for XGBoost configurations only, with the
    number of trees selected by early stopping and gain importance per
    feature; it is empty when no XGBoost model is requested.
    """
    if int(horizon) != 1:
        raise NotImplementedError("Esta fase solo soporta horizon=1")
    ...
    folds = make_expanding_folds(
        ...
        require_full_operational_days=require_full_operational_days,
    )
    ...
    return predictions, metrics_by_fold, metrics_comparison, folds_frame, diagnostics_frame
...
def run_multitarget_backtesting(
    ...
    """Run the same models for every configured target/scenario dataset."""
    pred_parts: list[pd.DataFrame] = []
    ...
    for scenario_name, dataset in datasets.items():
        ...
        predictions, _, _, folds, diagnostics = run_baseline_backtesting(
            dataset=dataset,
            model_names=model_names,
            run_id=run_id,
            test_weeks=test_weeks,
    ...
    diagnostics_frame = pd.concat(diagnostics_parts, ignore_index=True)
    return predictions, metrics_by_fold, metrics_comparison, folds_frame, diagnostics_frame
