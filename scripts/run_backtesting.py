...
from proyecto_grado.modeling.backtesting import (  # noqa: E402
    run_baseline_backtesting,
...
from proyecto_grado.modeling.splits import split_alignment_summary  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Directorio con datasets multitarget por escenario.",
    )
    ...
    parser.add_argument(
        "--versioned-outputs",
        action="store_true",
        help="Incluye run_id en tablas de salida para conservar resultados historicos.",
    )
    parser.add_argument(
        "--quiet",
        action="store_true",
...
def _csv_name(base_name: str, run_id: str, versioned: bool) -> str:
    """Return a CSV filename, optionally suffixed by run_id."""
    if not versioned:
        return f"{base_name}.csv"
    return f"{base_name}_{run_id}.csv"


def _load_multitarget_datasets(
    dataset_dir: Path,
    target_cfg,
    scenarios,
) -> tuple[dict[str, pd.DataFrame], dict[str, dict[str, str]], dict[str, str]]:
    """Load all scenario datasets and return backtesting specs."""
    datasets: dict[str, pd.DataFrame] = {}
    specs: dict[str, dict[str, str]] = {}
    dataset_paths: dict[str, str] = {}
    for scenario_name, scenario in scenarios.items():
        path = scenario_dataset_path(
            scenario,
        ...
        if not path.exists():
            raise FileNotFoundError(
                f"No existe dataset para escenario {scenario_name}: {path}. "
                "Ejecute build_modeling_dataset.py --multitarget primero."
            )
        target_def = target_definition(target_cfg, scenario.target)
        datasets[scenario_name] = pd.read_parquet(path)
        specs[scenario_name] = {
        ...
        dataset_paths[scenario_name] = str(path)
    return datasets, specs, dataset_paths
...
def main() -> None:
    ...
    if args.multitarget:
        ...
        if not args.quiet:
            print("\nRUN_BACKTESTING.PY - INICIO MULTITARGET")
            ...
            print(
                f"  dataset timestamp: {dataset['timestamp'].min()} -> {dataset['timestamp'].max()}"
            )
            print(f"  modelos cargados: {', '.join(model_names)}")
            print("  Nota: train y valid/test se separan por tiempo; no se usa shuffle.")

        predictions, metrics_by_fold, metrics_comparison, folds, _diagnostics = (
            run_multitarget_backtesting(
                datasets=datasets,
        ...
        outputs = {
            ...
            "metrics_by_fold": tables_dir
            / _csv_name("metrics_by_fold_multitarget", run_id, args.versioned_outputs),
            "metrics_comparison": tables_dir
            / _csv_name("metrics_comparison_multitarget", run_id, args.versioned_outputs),
            "folds": tables_dir
            / _csv_name("backtesting_folds_multitarget", run_id, args.versioned_outputs),
            "alignment_summary": tables_dir / f"split_boundary_exclusions_{run_id}.csv",
        }
    else:
        dataset = pd.read_parquet(dataset_path)
        dataset_paths = {"legacy": str(dataset_path)}
        if not args.quiet:
    ...
    alignment_summary = split_alignment_summary(
        dataset,
        test_weeks=args.test_weeks or bt_cfg.test_weeks,
        granularity_min=target_cfg.granularity_min,
        operational_day_start=bt_cfg.operational_day_start,
    ...
    backups = {
        ...
        "alignment_summary": _write_frame(
            alignment_summary,
            outputs["alignment_summary"],
            run_id,
        ),
    }
    test_rows = folds[folds["split"].eq("test_final")].copy()
    test_start_aligned = str(test_rows["eval_start"].iloc[0]) if not test_rows.empty else None
    test_end_aligned = str(test_rows["eval_end"].iloc[0]) if not test_rows.empty else None
    final_test_exclusions = alignment_summary[
        alignment_summary["scope"].eq("final_test_window")
    ].copy()
    excluded_partial_start_day = bool(
        final_test_exclusions.loc[
            final_test_exclusions["boundary"].eq("start"),
            "rows_excluded",
        ].sum()
        > 0
    )
    excluded_partial_end_day = bool(
        final_test_exclusions.loc[
            final_test_exclusions["boundary"].eq("end"),
            "rows_excluded",
        ].sum()
        > 0
    )
    ...
    run_metadata = {
        "run_id": run_id,
        "mode": "multitarget" if args.multitarget else "legacy",
        "dataset_path": str(dataset_path) if not args.multitarget else None,
        "dataset_paths": dataset_paths,
        "dataset_hash_sha256": file_sha256(dataset_path) if not args.multitarget else None,
        ...
        "split_alignment_policy": "full_operational_days_half_open",
        "operational_day_start": bt_cfg.operational_day_start,
        "require_full_operational_days": bt_cfg.require_full_operational_days,
        "drop_partial_boundary_days": bt_cfg.drop_partial_boundary_days,
        "test_start_aligned": test_start_aligned,
        "test_end_exclusive_aligned": test_end_aligned,
        "excluded_partial_start_day": excluded_partial_start_day,
        "excluded_partial_end_day": excluded_partial_end_day,
        "alignment_summary_path": str(outputs["alignment_summary"]),
        "models": model_names,
    ...
    metadata_path = (
        metadata_dir / f"modeling_multitarget_{run_id}.json"
        if args.multitarget
        else metadata_dir / f"run_{run_id}.json"
    )
    ...
    print(f"run_id: {run_id}")
    print(f"dataset: {dataset_path if not args.multitarget else dataset_dir}")
    ...
    print("\nresumen alineacion dias operativos completos:")
