...
def build_error_tables(predictions: pd.DataFrame) -> dict[str, pd.DataFrame]:
    ...
    return {
        "by_route": metrics_by_group(predictions, [*prefix_cols, "split", "model", "route"]),
        "by_hour": metrics_by_group(predictions, [*prefix_cols, "split", "model", "hora_del_dia"]),
        "by_day_type": metrics_by_group(predictions, [*prefix_cols, "split", "model", "tipo_dia"]),
        "by_target": metrics_by_group(predictions, ["target", "split", "model"])
        if "target" in predictions.columns
        else pd.DataFrame(),
        "by_scenario": metrics_by_group(predictions, ["target", "scenario", "split", "model"])
        if {"target", "scenario"}.issubset(predictions.columns)
        else pd.DataFrame(),
        "by_dispatch_history_level": metrics_by_group(
            predictions.dropna(subset=["dispatch_history_level"]),
            [*prefix_cols, "split", "model", "dispatch_history_level"],
        )
        if "dispatch_history_level" in predictions.columns
        else pd.DataFrame(),
    }
